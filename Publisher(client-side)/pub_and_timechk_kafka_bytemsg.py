...
for i in generatorObj:
    #transferSlice = zlib.compress(pickle.dumps(dataset[i:i + rowBatchSize]))
    transferSlice = dataset[i:i + rowBatchSize].to_json(orient='records', double_precision=15)
    print("ex batch size:", sys.getsizeof(transferSlice))
